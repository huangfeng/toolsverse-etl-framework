...
/**
 * This {@link Task#POST} task performs a union operation on two data sets. 
 *
...
public class Union implements OnTask
{
    /** The UNION variable. Defines name of the source to union with. */
    public static final String UNION_VAR = "UNION";
    
    /** The type of union variable **/
    public static final String UNION_TYPE_VAR = "TYPE";
        ...
        throws Exception
    {
        ...
        String sourceName = task.getVariable(UNION_VAR).getValue();
        ...
        String unionType = task.getVariable(UNION_TYPE_VAR).getValue().trim();
        
        String keys = null;
        
        if (UNION.equalsIgnoreCase(unionType))
        {
            isUnionAll = false;
            
            keys = task.getVariable(EtlConfig.KEYS_VAR).getValue();
        }
        
        dataSet = CommonEtlUtils.union(dataSet, source.getDataSet(), keys,
                isUnionAll, include, exclude);
        ...
        throws Exception
    {
        ...
        EtlUtils.addVar(task.getVariables(), UNION_VAR, "", false);
        EtlUtils.addVar(task.getVariables(), UNION_TYPE_VAR, "", false);
